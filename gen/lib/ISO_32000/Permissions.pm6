...
- (Optional) An indirect reference to a signature dictionary. This dictionary shall contain a Reference entry that is a signature reference dictionary that has a DocMDP transform method and corresponding transform parameters.
...
The signature dictionary shall contain a Reference entry that is a signature reference dictionary that has a UR transform method. The transform parameter dictionary for this method indicates which additional permissions is granted for the document. If the signature is valid, the conforming reader shall allow the specified permissions for the document, in addition to the application’s default permissions.
