use v6;
# generated by: ../../etc/make-modules.p6 --role-name=ISO_32000::Image ../tables/Image_entries.html
...
=head2 Intent [name]
- (Optional; PDF 1.1) The name of a colour rendering intent to be used in rendering the image (see Link 8.6.5.8, "Rendering Intents" ). Default value: the current rendering intent in the graphics state.

=head2 ImageMask [boolean]
- (Optional) A flag indicating whether the image is treated as an image mask (see Link 8.9.6, "Masked Images" ). If this flag is true, the value of BitsPerComponent is 1 and Mask and ColorSpace shall not be specified; unmasked areas shall bepainted using the current nonstroking colour. Default value: false.

=head2 Mask [stream or array]
- (Optional except for image masks; not allowed for image masks; PDF 1.3) An image XObject defining an image mask to be applied to this image (see Link 8.9.6.3, "Explicit Masking" ), or an array specifying a range of colours to be applied to it as a colour key mask (see Link 8.9.6.4, "Colour Key Masking" ). If ImageMask is true, this entry shall not be present.

=head2 Decode [array]
- (Optional) An array of numbers describing how to map image samples into the range of values appropriate for the image’s colour space (see Link 8.9.5.2, "Decode Arrays" ). If ImageMask is true, the array is either [ 0 1 ] or [ 1 0 ]; otherwise, its length shall betwice the number of colour components required by ColorSpace. If the image uses the JPXDecode filter and ImageMask is false, Decode is ignored by a conforming reader.
Default value: see Link 8.9.5.2, "Decode Arrays" .

=head2 Interpolate [boolean]
- (Optional) A flag indicating whether image interpolation shall beperformed by a conforming reader (see Link 8.9.5.3, "Image Interpolation" ). Default value: false.

=head2 Alternates [array]
- (Optional; PDF 1.3) An array of alternate image dictionaries for this image (see Link 8.9.5.4, "Alternate Images" ). The order of elements within the array shall have no significance. This entry shall not bepresent in an image XObject that is itself an alternate image.
...
- (Optional for images that use the JPXDecode filter, meaningless otherwise; PDF 1.5) A code specifying how soft-mask information (see Link 11.6.5.3, "Soft-Mask Images" ) encoded with image samples is used:
...
=head2 Name [name]
- (Required in PDF 1.0; optional otherwise) The name by which this image XObject is referenced in the XObject subdictionary of the current resource dictionary (see Link 7.8.3, "Resource Dictionaries" ).
This entry is obsolescent and shall no longer be used.

=head2 StructParent [integer]
- (Required if the image is a structural content item; PDF 1.3) The integer key of the image’s entry in the structural parent tree (see Link 14.7.4.4, "Finding Structure Elements from Content Items" ).

=head2 ID [byte string]
- (Optional; PDF 1.3; indirect reference preferred) The digital identifier of the image’s parent Web Capture content set (see Link 14.10.6, "Object Attributes Related to Web Capture" ).

=head2 OPI [dictionary]
- (Optional; PDF 1.2) An OPI version dictionary for the image; see Link 14.11.7, "Open Prepress Interface (OPI)" . If ImageMask is true, this entry is ignored.

=head2 Metadata [stream]
- (Optional; PDF 1.4) A metadata stream containing metadata for the image (see Link 14.3.2, "Metadata Streams" ).

=head2 OC [dictionary]
- (Optional; PDF 1.5) An optional content group or optional content membership dictionary (see Link 8.11, "Optional Content" ), specifying the optional content properties for this image XObject. Before the image is processed by a conforming reader, its visibility shall bedetermined based on this entry. If it is determined to be invisible, the entire image is skipped, as if there were no Do operator to invoke it.
