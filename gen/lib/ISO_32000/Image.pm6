...
role ISO_32000::Image {
    method Type {...};	#| [name] (Optional) The type of PDF object that this dictionary describes; if present, is XObject for an image XObject.
    method Subtype {...};	#| [name] (Required) The type of XObject that this dictionary describes; is Image for an image XObject.
    ...
    method ColorSpace {...};	#| [name or array] (Required for images, except those that use the JPXDecode filter; not allowed forbidden for image masks) The colour space in which image samples is specified; it can be any type of colour space except Pattern.
    ...
    method BitsPerComponent {...};	#| [integer] (Required except for image masks and images that use the JPXDecode filter) The number of bits used to represent each colour component. Only a single value is specified; the number of bits is the same for all colour components. The value is 1, 2, 4, 8, or (in PDF 1.5) 16. If ImageMask is true, this entry is optional, but if specified, its value is 1.
    ...
    method ImageMask {...};	#| [boolean] (Optional) A flag indicating whether the image is treated as an image mask (see Link 8.9.6, "Masked Images" ). If this flag is true, the value of BitsPerComponent is 1 and Mask and ColorSpace shall not be specified; unmasked areas shall bepainted using the current nonstroking colour. Default value: false.
    ...
    method SMaskInData {...};	#| [integer] (Optional for images that use the JPXDecode filter, meaningless otherwise; PDF 1.5) A code specifying how soft-mask information (see Link 11.6.5.3, "Soft-Mask Images" ) encoded with image samples is used:
    ...
    method OPI {...};	#| [dictionary] (Optional; PDF 1.2) An OPI version dictionary for the image; see Link 14.11.7, "Open Prepress Interface (OPI)" . If ImageMask is true, this entry is ignored.
