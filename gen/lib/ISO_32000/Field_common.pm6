...
role ISO_32000::Field_common {
    ...
    method TU {...};	#| [text string] (Optional; PDF 1.3) An alternate field name that is used in place of the actual field name wherever the field is identified in the user interface (such as in error or status messages referring to the field). This text is also useful when extracting the document’s contents in support of accessibility to users with disabilities or for other purposes (see Link 14.9.3, “Alternate Descriptions” ).
    method TM {...};	#| [text string] (Optional; PDF 1.3) The mapping name that is used when exporting interactive form field data from the document.
