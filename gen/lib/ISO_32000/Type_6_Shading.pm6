...
role ISO_32000::Type_6_Shading {
    method BitsPerCoordinate {...};	#| [integer] (Required) The number of bits used to represent each geometric coordinate. The value is 1, 2, 4, 8, 12, 16, 24, or 32.
    ...
    method BitsPerFlag {...};	#| [integer] (Required) The number of bits used to represent the edge flag for each patch (see below). The value is 2, 4, or 8, but only the least significant 2 bits in each flag value is used. Valid values for the edge flag is 0, 1, 2, and 3.
    method Decode {...};	#| [array] (Required) An array of numbers specifying how to map coordinates and colour components into the appropriate ranges of values. The decoding method is similar to that used in image dictionaries (see Link 8.9.5.2, "Decode Arrays" ). The ranges is specified as follows:
