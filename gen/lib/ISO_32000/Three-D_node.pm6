...
=head2 N [text string]
- (Required) The name of the node being described by the node dictionary. If the Subtype of the corresponding 3D Stream is U3D, this entry corresponds to the field Node block name, as described in the Universal 3D file formatspecification (see Link Bibliography ). In the future, nodes may be described using other 3D conventions.
...
=head2 M [array]
- (Optional) A 12-element 3D transformation matrix that specifies the position and orientation of this node, relative to its parent, in world coordinates (see Link 13.6.5, “Coordinate Systems for 3D” ).
