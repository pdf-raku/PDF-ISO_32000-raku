...
=head2 Subtype [name]
- (Required) The type of render mode described by this dictionary. If an unrecognized value is encountered, then this render mode dictionary is ignored.
...
If the value of FC is a name, it shall describe a colour. The only valid name value shall BG, specifying the current background colour in use for displaying the artwork. If a name other than BG is encountered, this entry is ignored and the background colour for the host annotation is used.
Default value: BG
