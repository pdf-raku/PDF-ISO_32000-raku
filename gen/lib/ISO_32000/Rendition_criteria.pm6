...
=head2 D [dictionary]
- (Optional) A dictionary specifying the minimum bit depth required in order for this object to be viable.
NOTE 4 Equivalent to SMIL’s systemScreenDepth attribute.

=head2 Z [dictionary]
- (Optional) A dictionary specifying the minimum screen size required in order for this object to be viable.
NOTE 5 Equivalent to SMIL’s systemScreenSize attribute.

=head2 V [array]
- (Optional) An array of software identifier objects. If this entry is present and non-empty, the conforming reader is identified by one or more of the objects in the array in order for this object to be viable.

=head2 P [array]
- (Optional) An array containing one or two name objects specifying a minimum and optionally a maximum PDF language version, in the same format as the Version entry in the document catalog. If this entry is present and non-empty, the version of multimedia constructs fully supported by the conforming reader is within the specified range in order for this object to be viable.

=head2 L [array]
- (Optional) An array of language identifiers. If this entry is present and non-empty, the language in which the conforming reader is running shall exactly match a language identifier, or consist only of a primary code that matches the primary code of an identifier, in order for this object to be viable.
