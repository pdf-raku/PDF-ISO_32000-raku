...
=head2 LW [number]
- (Optional; PDF 1.3) The line width (see Link 8.4.3.2, "Line Width" ).

=head2 LC [integer]
- (Optional; PDF 1.3) The line cap style (see Link 8.4.3.3, "Line Cap Style" ).

=head2 LJ [integer]
- (Optional; PDF 1.3) The line join style (see Link 8.4.3.4, "Line Join Style" ).

=head2 ML [number]
- (Optional; PDF 1.3) The miter limit (see Link 8.4.3.5, "Miter Limit" ).

=head2 D [array]
- (Optional; PDF 1.3) The line dash pattern, expressed as an array of the form [ dashArray dashPhase ], where dashArray is itself an array and dashPhase is an integer (see Link 8.4.3.6, "Line Dash Pattern" ).

=head2 RI [name]
- (Optional; PDF 1.3) The name of the rendering intent (see Link 8.6.5.8, "Rendering Intents" ).

=head2 OP [boolean]
- (Optional) A flag specifying whether to apply overprint (see Link 8.6.7, "Overprint Control" ). In PDF 1.2 and earlier, there is a single overprint parameter that applies to all painting operations. Beginning with PDF 1.3, there is two separate overprint parameters: one for stroking and one for all other painting operations. Specifying an OP entry shall set both parameters unless there is also an op entry in the same graphics state parameter dictionary, in which case the OP entry shall set only the overprint parameter for stroking.

=head2 op [boolean]
- (Optional; PDF 1.3) A flag specifying whether to apply overprint (see Link 8.6.7, "Overprint Control" ) for painting operations other than stroking. If this entry is absent, the OP entry, if any, shall also set this parameter.

=head2 OPM [integer]
- (Optional; PDF 1.3) The overprint mode (see Link 8.6.7, "Overprint Control" ).
...
=head2 BG [function]
- (Optional) The black-generation function, which maps the interval [ 0.0 1.0 ] to the interval [ 0.0 1.0 ] (see Link 10.3.4, "Conversion from DeviceRGB to DeviceCMYK" ).
...
=head2 UCR [function]
- (Optional) The undercolor-removal function, which maps the interval [ 0.0 1.0 ] to the interval [ −1.0 1.0 ] (see Link 10.3.4, "Conversion from DeviceRGB to DeviceCMYK" ).
...
- (Optional) The halftone dictionary or stream (see Link 10.5, "Halftones" ) or the name Default, denoting the halftone that was in effect at the start of the page.

=head2 FL [number]
- (Optional; PDF 1.3) The flatness tolerance (see Link 10.6.2, "Flatness Tolerance" ).

=head2 SM [number]
- (Optional; PDF 1.3) The smoothness tolerance (see Link 10.6.3, "Smoothness Tolerance" ).

=head2 SA [boolean]
- (Optional) A flag specifying whether to apply automatic stroke adjustment (see Link 10.6.5, "Automatic Stroke Adjustment" ).

=head2 BM [name or array]
- (Optional; PDF 1.4) The current blend mode to be used in the transparent imaging model (see Link 11.3.5, "Blend Mode" and Link 11.6.3, "Specifying Blending Colour Space and Blend Mode" ).

=head2 SMask [dictionary or name]
- (Optional; PDF 1.4) The current soft mask, specifying the mask shape or mask opacity values that is used in the transparent imaging model (see Link 11.3.7.2, "Source Shape and Opacity" and Link 11.6.4.3, "Mask Shape and Opacity" ).
...
=head2 CA [number]
- (Optional; PDF 1.4) The current stroking alpha constant, specifying the constant shape or constant opacity value that is used for stroking operations in the transparent imaging model (see Link 11.3.7.2, "Source Shape and Opacity" and Link 11.6.4.4, "Constant Shape and Opacity" ).
...
=head2 TK [boolean]
- (Optional; PDF 1.4) The text knockout flag, shall determine the behaviour of overlapping glyphs within a text object in the transparent imaging model (see Link 9.3.8, "Text Knockout" ).
