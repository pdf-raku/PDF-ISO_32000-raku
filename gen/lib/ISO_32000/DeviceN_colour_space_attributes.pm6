use v6;
# generated by: ../../etc/make-modules.p6 --role-name=ISO_32000::DeviceN_colour_space_attributes ../../resources/ISO_32000/DeviceN_colour_space_attributes_entries.json
...
- (Required if Subtype is NChannel and the colour space includes components of a process colour space, otherwise optional; PDF 1.6) A dictionary that describes the process colour space whose components are included in this colour space.

=head2 MixingHints [dictionary]
- (Optional; PDF 1.6) A dictionary that specifies optional attributes of the inks that is used in blending calculations when used as an alternative to the tint transformation function.
