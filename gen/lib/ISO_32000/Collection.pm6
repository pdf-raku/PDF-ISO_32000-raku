use v6;
# generated by: ../../etc/make-modules.p6 --role-name=ISO_32000::Collection ../tables/Collection_entries.html
...
- (Optional) A collection schema dictionary (see Link Ta b l e 156 ). If absent, the conforming reader may choose useful defaults that are known to exist in a file specification dictionary, such as the file name, file size, and modified date.
...
- (Optional) A collection sort dictionary, which specifies the order in which items in the collection is sorted in the user interface (see Link Ta b l e 158 ).
