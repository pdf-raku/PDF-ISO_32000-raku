use v6;
# generated by: ../../etc/make-modules.p6 --role-name=ISO_32000::Three-D_view ../tables/Three-D_view_entries.html
...
- (Optional) The internal name of the view, used to refer to the view from other objects, such as the go-to-3D-view action (see Link 12.6.4.15, “Go-To- 3D-View Actions” ).
...
=head2 P [dictionary]
- (Optional) A projection dictionary (see Link 13.6.4.2, “Projection Dictionaries” ) that defines the projection of coordinates in the 3D artwork (already transformed into camera coordinates) onto the target coordinate system of the annotation.
...
=head2 O [stream]
- (Optional; meaningful only if MS and P are present) A form XObject that is used to overlay 2D graphics on top of the rendered 3D artwork (see Link 13.6.6, “3D Markup” ).

=head2 BG [dictionary]
- (Optional) A background dictionary that defines the background over which the 3D artwork is drawn (see Link 13.6.4.3, “3D Background Dictionaries” ). Default value: a background dictionary whose entries take their default values.

=head2 RM [dictionary]
- (Optional; PDF 1.7) A render mode dictionary that specifies the render mode to use when rendering 3D artwork with this view (see Link 13.6.4.4, “3D Render Mode Dictionaries” ). If omitted, the render mode specified in the 3D artwork is used.

=head2 LS [dictionary]
- (Optional; PDF 1.7) A lighting scheme dictionary that specifies the lighting scheme to be used when rendering 3D artwork with this view (see Link 13.6.4.5, “3D Lighting Scheme Dictionaries” ). If omitted, the lighting scheme specified in the 3D artwork is used.

=head2 SA [array]
- (Optional; PDF 1.7) An array that contains cross section dictionaries (see Link 13.6.4.6, “3D Cross Section Dictionaries” ). Each cross section dictionary provides parameters for applying a cross section to the 3D artwork when using this view. An empty array signifies that no cross sections is displayed.

=head2 NA [array]
- (Optional; PDF 1.7; meaningful only if NR is present) An array that contains 3D node dictionaries (see Link 13.6.4.7, “3D Node Dictionaries” ). Each node dictionary may contain entries that change the node’s state, including its opacity and its position in world space. This entry and the NR entry specify how the state of each node is changed.
