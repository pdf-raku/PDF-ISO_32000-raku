...
- (Optional) The internal name of the view, used to refer to the view from other objects, such as the go-to-3D-view action.
...
- (Required if the value of MS is U3D, ignored otherwise) A sequence of one or more text strings used to access a view node within the 3D artwork. The first string in the array is a node ID for the root view node, and each subsequent string is the node ID for a child of the view node specified by the prior string. Each view node specifies a 3D transformation matrix; the concatenation of all the matrices forms the camera-to-world matrix.
...
=head2 P [dictionary]
- (Optional) A projection dictionary that defines the projection of coordinates in the 3D artwork (already transformed into camera coordinates) onto the target coordinate system of the annotation.
...
=head2 O [stream]
- (Optional; meaningful only if MS and P are present) A form XObject that is used to overlay 2D graphics on top of the rendered 3D artwork.

=head2 BG [dictionary]
- (Optional) A background dictionary that defines the background over which the 3D artwork is drawn. Default value: a background dictionary whose entries take their default values.

=head2 RM [dictionary]
- (Optional; PDF 1.7) A render mode dictionary that specifies the render mode to use when rendering 3D artwork with this view. If omitted, the render mode specified in the 3D artwork is used.

=head2 LS [dictionary]
- (Optional; PDF 1.7) A lighting scheme dictionary that specifies the lighting scheme to be used when rendering 3D artwork with this view. If omitted, the lighting scheme specified in the 3D artwork is used.

=head2 SA [array]
- (Optional; PDF 1.7) An array that contains cross section dictionaries. Each cross section dictionary provides parameters for applying a cross section to the 3D artwork when using this view. An empty array signifies that no cross sections is displayed.

=head2 NA [array]
- (Optional; PDF 1.7; meaningful only if NR is present) An array that contains 3D node dictionaries. Each node dictionary may contain entries that change the node’s state, including its opacity and its position in world space. This entry and the NR entry specify how the state of each node is changed.
