...
=head2 Matrix [array]
- (Optional) An array of six numbers specifying the form matrix, which maps form space into user space (see Link 8.3.4, "Transformation Matrices" ). Default value: the identity matrix [ 1 0 0 1 0 0 ].

=head2 Resources [dictionary]
- (Optional but strongly recommended; PDF 1.2) A dictionary specifying any resources (such as fonts and images) required by the form XObject (see Link 7.8, "Content Streams and Resources" ).
...
- (Optional; PDF 1.4) A group attributes dictionary indicating that the contents of the form XObject is treated as a group and specifying the attributes of that group (see Link 8.10.3, "Group XObjects" ).
...
- (Optional; PDF 1.4) A reference dictionary identifying a page to be imported from another PDF file, and for which the form XObject serves as a proxy (see Link 8.10.4, "Reference XObjects" ).

=head2 Metadata [stream]
- (Optional; PDF 1.4) A metadata stream containing metadata for the form XObject (see Link 14.3.2, "Metadata Streams" ).

=head2 PieceInfo [dictionary]
- (Optional; PDF 1.3) A page-piece dictionary associated with the form XObject (see Link 14.5, "Page-Piece Dictionaries" ).

=head2 LastModified [date]
- (Required if PieceInfo is present; optional otherwise; PDF 1.3) The date and time (see Link 7.9.4, "Dates" ) when the form XObject’s contents were most recently modified. If a page-piece dictionary (PieceInfo) is present, the modification date is used to ascertain which of the application data dictionaries it contains correspond to the current content of the form (see Link 14.5, "Page-Piece Dictionaries" ).

=head2 StructParent [integer]
- (Required if the form XObject is a structural content item; PDF 1.3)The integer key of the form XObject’s entry in the structural parent tree (see Link 14.7.4.4, "Finding Structure Elements from Content Items" ).

=head2 StructParents [integer]
- (Required if the form XObject contains marked-content sequences that are structural content items; PDF 1.3) The integer key of the form XObject’s entry in the structural parent tree (see Link 14.7.4.4, "Finding Structure Elements from Content Items" ).
...
- (Optional; PDF 1.2) An OPI version dictionary for the form XObject (see Link 14.11.7, "Open Prepress Interface (OPI)" ).

=head2 OC [dictionary]
- (Optional; PDF 1.5) An optional content group or optional content membership dictionary (see Link 8.11, "Optional Content" ) specifying the optional content properties for the form XObject. Before the form is processed, its visibility is determined based on this entry. If it is determined to be invisible, the entire form is skipped, as if there were no Do operator to invoke it.

=head2 Name [name]
- (Required in PDF 1.0; optional otherwise) The name by which this form XObject is referenced in the XObject subdictionary of the current resource dictionary (see Link 7.8.3, "Resource Dictionaries" ).
