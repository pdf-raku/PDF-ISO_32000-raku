...
- (Optional; shall not be present if an A entry is present) The destination that is displayed when this item is activated (see Link 12.3.2, “Destinations” ).

=head2 A [dictionary]
- (Optional; PDF 1.1; shall not be present if a Dest entry is present) The action that is performed when this item is activated (see Link 12.6, “Actions” ).

=head2 SE [dictionary]
- (Optional; PDF 1.3; is an indirect reference) The structure element to which the item refers (see Link 14.7.2, “Structure Hierarchy” ).
...
=head2 F [integer]
- (Optional; PDF 1.4) A set of flags specifying style characteristics for displaying the outline item’s text (see Link Table 154 ). Default value: 0.
