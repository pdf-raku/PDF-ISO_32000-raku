...
=head2 A [dictionary]
- (Optional; PDF 1.1) An action that is performed when the link annotation is activated.

=head2 Dest [array, name or byte string]
- (Optional; not permitted if an A entry is present) A destination that is displayed when the annotation is activated.
...
=head2 BS [dictionary]
- (Optional; PDF 1.6) A border style dictionary specifying the line width and dash pattern to be used in drawing the annotation’s border.
