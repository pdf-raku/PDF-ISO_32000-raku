use v6;
# generated by: ../../etc/make-modules.p6 --role-name=ISO_32000::Type_3_Font ../tables/Type_3_Font_entries.html
...
=head2 Name [name]
- (Required in PDF 1.0; optional otherwise) See Link Table 111 .

=head2 FontBBox [rectangle]
- (Required) A rectangle (see Link 7.9.5, "Rectangles" ) expressed in the glyph coordinate system, specifying the font bounding box. This is the smallest rectangle enclosing the shape that would result if all of the glyphs of the font were placed with their origins coincident and then filled.
...
- (Required) An array of six numbers specifying the font matrix, mapping glyph space to text space (see Link 9.2.4, "Glyph Positioning and Metrics" ).
...
- (Required) An encoding dictionary whose Differences array shall specify the complete character encoding for this font (see Link 9.6.6, "Character Encoding" ).
...
- (Required in Tagged PDF documents; is an indirect reference) A font descriptor describing the font’s default metrics other than its glyph widths (see Link 9.8, "Font Descriptors" ).

=head2 Resources [dictionary]
- (Optional but should be used; PDF 1.2) A list of the named resources, such as fonts and images, required by the glyph descriptions in this font (see Link 7.8.3, "Resource Dictionaries" ). If any glyph descriptions refer to named resources but this dictionary is absent, the names is looked up in the resource dictionary of the page on which the font is used.

=head2 ToUnicode [stream]
- (Optional; PDF 1.2) A stream containing a CMap file that maps character codes to Unicode values (see Link 9.10, "Extraction of Text Content" ).
