...
role ISO_32000::Type_3_Font {
    ...
    method Subtype {...};	#| [name] (Required) The type of font; is Type3 for a Type 3 font.
    ...
    method CharProcs {...};	#| [dictionary] (Required) A dictionary in which each key is a glyph name and the value associated with that key is a content stream that constructs and paints the glyph for that character. The stream shall include as its first operator either d0 or d1, followed by operators describing one or more graphics objects, which may include path, text, or image objects. See below for more details about Type 3 glyph descriptions.
    ...
    method FontDescriptor {...};	#| [dictionary] (Required in Tagged PDF documents; is an indirect reference) A font descriptor describing the font’s default metrics other than its glyph widths (see Link 9.8, "Font Descriptors" ).
