...
- (Required) A rectangle expressed in the glyph coordinate system, specifying the font bounding box. This is the smallest rectangle enclosing the shape that would result if all of the glyphs of the font were placed with their origins coincident and then filled.
...
- (Required) An array of six numbers specifying the font matrix, mapping glyph space to text space.
...
- (Required) An encoding dictionary whose Differences array shall specify the complete character encoding for this font.
...
- (Required in Tagged PDF documents; is an indirect reference) A font descriptor describing the font’s default metrics other than its glyph widths.

=head2 Resources [dictionary]
- (Optional but should be used; PDF 1.2) A list of the named resources, such as fonts and images, required by the glyph descriptions in this font. If any glyph descriptions refer to named resources but this dictionary is absent, the names is looked up in the resource dictionary of the page on which the font is used.
