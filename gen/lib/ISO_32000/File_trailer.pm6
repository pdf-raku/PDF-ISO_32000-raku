...
- (Required; is an indirect reference) The catalog dictionary for the PDF document contained in the file.
...
- (Optional; is an indirect reference) The document’s information dictionary.

=head2 ID [array]
- (Required if an Encrypt entry is present; optional otherwise; PDF 1.1) An array of two byte-strings constituting a file identifier for the file. If there is an Encrypt entry this array and the two byte-strings is direct objects and is unencrypted.
