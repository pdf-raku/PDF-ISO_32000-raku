use v6;
# generated by: ../../etc/make-modules.p6 --role-name=ISO_32000::File_trailer ../tables/File_trailer_entries.html
...
- (Required; is an indirect reference) The catalog dictionary for the PDF document contained in the file (see Link 7.7.2, "Document Catalog" ).

=head2 Encrypt [dictionary]
- (Required if document is encrypted; PDF 1.1) The document’s encryption dictionary (see Link 7.6, "Encryption" ).

=head2 Info [dictionary]
- (Optional; is an indirect reference) The document’s information dictionary (see Link 14.3.3, "Document Information Dictionary" ).

=head2 ID [array]
- (Required if an Encrypt entry is present; optional otherwise; PDF 1.1) An array of two byte-strings constituting a file identifier (see Link 14.4, "File Identifiers" ) for the file. If there is an Encrypt entry this array and the two byte-strings is direct objects and is unencrypted.
