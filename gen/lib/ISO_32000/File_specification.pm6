...
Link 7.11.2, "File Specification Strings."
This is a text string encoded using PDFDocEncoding or UTF-16BE with a leading byte-order marker. The F entry should be included along with this entry for backwards compatibility reasons.

=head2 DOS [byte string]
- (Optional) A file specification string representing a DOS file name.
This entry is obsolescent and should not be used by conforming writers.

=head2 Mac [byte string]
- (Optional) A file specification string representing a Mac OS file name.
This entry is obsolescent and should not be used by conforming writers.

=head2 Unix [byte string]
- (Optional) A file specification string representing a UNIX file name.
This entry is obsolescent and should not be used by conforming writers.

=head2 ID [array]
- (Optional) An array of two byte strings constituting a file identifier that should be included in the referenced file.
...
=head2 EF [dictionary]
- (Required if RF is present; PDF 1.3; amended to include the UF key in PDF 1.7) A dictionary containing a subset of the keys F, UF, DOS, Mac, and Unix, corresponding to the entries by those names in the file specification dictionary. The value of each such key is an embedded file stream containing the corresponding file. If this entry is present, the Type entry is required and the file specification dictionary is indirectly referenced.
The F and UF entries should be used in place of the DOS, Mac, or Unixentries.

=head2 RF [dictionary]
- (Optional; PDF 1.3) A dictionary with the same structure as the EF dictionary, which is present. Each key in the RF dictionary shall also be present in the EF dictionary. Each value is a related files array identifying files that are related to the corresponding file in the EF dictionary. If this entry is present, the Type entry is required and the file specification dictionary is indirectly referenced.

=head2 Desc [text string]
- (Optional; PDF 1.6) Descriptive text associated with the file specification. It is used for files in the EmbeddedFiles name tree.

=head2 CI [dictionary]
- (Optional; is indirect reference; PDF 1.7) A collection item dictionary, which is used to create the user interface for portable collections.
